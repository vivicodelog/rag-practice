...
class TestListSessions:
    ...
    def test_list_sessions_empty(self, client):
        """没有会话时返回空列表"""
        resp = client.get("/sessions?mode=nl2sql")
        assert resp.status_code == 200
        assert resp.json() == []
...
class TestGetSession:
    ...
    def test_get_session_not_found(self, client):
        """不存在的 id 返回 404"""
        resp = client.get("/sessions/non-existent")
...
class TestDeleteSession:
    ...
    def test_delete_session_idempotent(self, client):
        """删不存在的 id 不会报错"""
        resp = client.delete("/sessions/ghost-id")
        assert resp.status_code == 200
