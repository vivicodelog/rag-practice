...
- ✅ **Agent 智能问答**：LLM 自主选择工具（搜文档/查天气），不固定流程
- ✅ **混合检索**：向量语义搜索 + 关键词匹配，结果更精准
- ✅ **工具调用**：支持 `search_docs`（文档搜索）+ `get_weather`（天气查询），可扩展
...
用户提问
    │
    ▼
FastAPI → 绑工具（search_docs / get_weather）
    │
    ▼
┌─ Agent 循环（最多 3 轮）──────────────┐
│  LLM 自主判断 → 调工具 → 执行 → 看结果 │
...
└──────────────────────────────────────┘
    │
    ▼
DeepSeek Chat LLM → 综合工具结果生成回答
    │
    ▼
返回 JSON → Vue 前端渲染气泡对话（含来源标签）
```
